...
interface CountryCardProps {
  ...
  // 🟢 UPDATED: we now pass selection instead of direct gift send
...
export function CountryCard({
  ...
  const barWidth =
    maxScore > 0
      ? Math.max((score / maxScore) * 100, score > 0 ? 2 : 0)
      : 0;
  ...
  return (
    <div
      className={`relative flex items-center gap-3 px-3 py-2.5 rounded-xl border transition-all duration-500 cursor-pointer group
        ${
          isLeader
            ? "border-yellow-400/60 bg-gradient-to-r from-yellow-900/20 to-transparent"
      ...
      onClick={() => {
        if (phase === "running") {
          setSelectedCountry(countryId); // 🟢 ONLY SELECT COUNTRY
        }
      }}
    >
      {/* Rank */}
      <div
        className={`w-7 text-center font-bold text-sm flex-shrink-0 transition-all
        ${
          rank === 1
            ? "text-yellow-400"
            : rank === 2
